...
class IpLocation {
    ...
    private function getAddr($ip) {
        ...
        if (!$this->isValidIpV4($ip)) {
            ...
            $result['data']['country'] = '';
            $result['data']['isp'] = '';
            $result['data']['area'] = "错误或不完整的IP地址: $ip";
            return $result;
        } else {
            ...
            $result['code'] = 200;
            //$result['data']['ip'] = $location['ip'];

            //$result['data']['range']['start'] = $location['beginip'];
            //$result['data']['range']['end'] = $location['endip'];

            //$result['org_country']    = $location['org_country'];  //纯真数据库返回的列1
            //$result['org_area'] = $location['org_area'];
            //$result['data']['country'] =  $location['country'] . $location['province'] . $location['city'] . $location['county'];

            //$result['data']['country'] = $location['country'] . $location['province'] . $location['city'] . $location['county'];
            //$result['province'] = $location['province'];
            //$result['city']     = $location['city'];
            //$result['county']   = $location['county'];
            //$result['data']['isp'] = $location['isp'];
            if($location['org_country'] !== $location['country']){
                $location['org_country'] = $location['org_country'];    
             }else{
                $location['org_country'] = '';    
             }

            if($location['country']=='本机地址'){
              //$result['msg'] = '本机地址';
              $result['data']['local'] = '本机地址';
              $result['data']['isp'] = '本机地址';
            }elseif($location['country']=='局域网'){
            $result['data']['local'] = '局域网';
            $result['data']['isp'] = '局域网';
            }else{
            $result['data']['local'] = $location['country'].$location['org_country'];
            $result['data']['isp'] = $location['org_area'];
            $result['area'] = $location['country'].$location['org_country'].$location['org_area']; 
            }
            //$result['data']['area'] = $location['country'] . $location['province'] . $location['city'] . $location['county'] . $location['org_area'];
        }
        return $result; //array
    }
